...
How a release of `scaffld` is made, what evidence it leaves behind, how anyone
...
gh release download vX.Y.Z --repo ferinazumaDEV/scaffld
...
gh attestation verify ./*.whl --repo ferinazumaDEV/scaffld
gh attestation verify ./*.tar.gz --repo ferinazumaDEV/scaffld

# 3. PyPI serves those same bytes, and says where they came from.
curl -s https://pypi.org/pypi/scaffld/X.Y.Z/json \
...
curl -s -H 'Accept: application/vnd.pypi.integrity.v1+json' \
  https://pypi.org/integrity/scaffld/X.Y.Z/FILENAME/provenance \
...
```bash
gh run download <run-id> --repo ferinazumaDEV/scaffld --name dist --dir dist
gh release create vX.Y.Z --repo ferinazumaDEV/scaffld --title vX.Y.Z \
